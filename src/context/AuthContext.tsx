  ...
  const loginWithApi = async (email: string, password: string): Promise<boolean> => {
    try {
      ...
      if (response.ok && (data.success || data.token || data.usuario)) {
        const user: User = {
          id: data.usuario?.id || data.data?.usuario?._id || '1',
          email: email,
          name: data.usuario?.nombreUsr || data.data?.usuario?.nombreUsr || email.split('@')[0],
          lastName: data.usuario?.apellidoUsr || data.data?.usuario?.apellidoUsr || '',
          phone: data.usuario?.celUsr || data.data?.usuario?.celUsr || '',
          role: 'user',
        };
